...
class SimAccount extends Model
{
	 ...
	 * @return int
	 */
	public static function addMany($loginStr, $clientId)
	{
		$addCount = 0;
		...
		if(!preg_match_all('!(9\d{9})!', $loginStr, $res))
		{
		...
		{
			if(self::getModel(['login'=>$login]))
			{
				self::$lastError = 'кошелек уже добавлен '.$res[0][$key];
				continue;
			}
			...
			{
				self::$lastError = 'кошелек не добавлен '.$res[0][$key];
				return $addCount;
			}
		}
	...
	public function getTransactions($timestampStart, $timestampEnd, $type = '')
	{
		...
		if($type == 'successIn')
			$cond = " AND `amount`>0 AND `status`='".SimTransaction::STATUS_SUCCESS."'";
		elseif($type == 'out')
			$cond = " AND `amount`<0";
		else
			$cond = '';

		return SimTransaction::model()->findAll([
	 ...
	 * @return SimTransaction|bool
	 */
	public function addTransaction(array $params)
	{
		$model = new SimTransaction;
		...
		$model->scenario = ($params['amount'] > 0)
			? SimTransaction::SCENARIO_ADD
			: SimTransaction::SCENARIO_WITHDRAW;
	...
	{
		if(
			$model = SimTransaction::getModel(['account_id'=>$this->id, 'id'=>$id])
	...
	public function updateInfo()
	{
		...
		$model = SimTransaction::model()->find([
			'select' => "SUM(`amount`) AS 'transAmount'",
			'condition' => "`account_id`='{$this->id}' AND `status`='".SimTransaction::STATUS_SUCCESS."'",
		]);
		...
		$model = SimTransaction::model()->find([
			...
			'condition' => "`account_id`='{$this->id}'
				AND `status`='".SimTransaction::STATUS_WAIT."'",
		]);
		...
		$model = SimTransaction::model()->find([
			'select' => "SUM(`amount`) AS 'transAmount'",
			'condition' => "`account_id`='{$this->id}'
				AND `status`='".SimTransaction::STATUS_SUCCESS."'
		...
		{
			//debug
			//toLogRuntime('Sim кошелек обновлен +'.$this->login);
			return true;
		}
		else
			return false;
	...
	public function getTransactionCount($type='')
	{
		...
		elseif($type == 'successIn')
			$typeCond = " AND `amount` > 0 AND `status`='".SimTransaction::STATUS_SUCCESS."'";
		else
			$typeCond = '';

		return SimTransaction::model()->count("`account_id`='{$this->id}' AND `date_add`>=$timestampStart $typeCond");
	}
	...
	{
		$minAmount = SimTransaction::AMOUNT_MIN;
		$maxAmount = SimTransaction::AMOUNT_MAX;
...
//		}
	...
	{
		$transaction = SimTransaction::getModel(['id'=>$id, 'user_id'=>$userId]);
		...
		if($transaction->status == SimTransaction::STATUS_SUCCESS)
			return true;

		if($transaction->status != SimTransaction::STATUS_WAIT)
		{
			self::$msg = 'неверный статус платежа';
			return false;
		}

		$transaction->status = SimTransaction::STATUS_SUCCESS;

		if($transaction->save())
		{
			toLogRuntime('Sim: платеж подтвержден id='.$transaction->id.' '.$amount.' руб');
			return true;
		}
		else
			return false;
	...
	public function getAmountIn()
	{
		if($this->_amountInCache === null)
		{
			...
			$model = SimTransaction::model()->find([
				'select' => "SUM(`amount`) AS 'transAmount'",
				'condition' => "`account_id`='{$this->id}' AND `status`='".SimTransaction::STATUS_SUCCESS."' AND `amount` > 0 AND `date_add`>=$dateStart",
			]);
