...
class Email
{
	...
	*/
	...
	public $template;	//шаблон для письма, лежат в папке templates
	public $htmlSubject; //<title>...</title>
	...
	*/
	...
	{
		$this->htmlSubject = htmlspecialchars($subject);
	...
	*/
	function Body( $body, $text_html="" )
	{
		...
		if( $text_html == "html" )
        {
             ...
             $this->body = $this->buildHtml($this->body); 
        }
	...
	*/
	function Send()
	{
		...
		else // если через smtp
		{
			...
			$code = substr($data,0,3);
			if($code != 235) {$this->smtp_log .= "неправильный пароль\n"; fclose($smtp_conn); return ;}
	...
	*/
     ...
     * EMail::buildHtml()
     * k0nstant
     * Построит html-тело из контента
     * @param mixed $content
     * @return void
     */
    private function buildHtml($content)
    {
    	$tplFolder = dirname(__FILE__).'/'.__CLASS__.'/templates/';
    	$tplFile = $tplFolder.$this->template.'.php';
    	
    	if($this->template and file_exists($tplFile))
    	{
    		ob_start();
			include($tplFile);
