...
class MerchantTransaction extends Model
{
	...
	public function rules()
	{
		return [
			...
			['status, error, currency, date_add, commision, date_add_db, merchant_wallet_id, transaction_id', 'safe'],
		];
...
//			toLogRuntime(arr2str($transactions));
			...
			foreach($transactions as $key=>$transaction)
			{
				if($transaction['src_user'] !== 'SYSTEM' and ($transaction['protocol_type'] == 'TRANSFER'  or  $transaction['protocol_type'] == 'CARD'))
				{
					...
					$paymentsArr[$key]['currency'] = $transaction['currency'];
...
//			prrd($paymentsArr);
	...
	{
		return formatAmount($this->amount, 2).($this->currency ? $this->currency : ' руб');
	}
