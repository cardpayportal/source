...
class AccountController extends Controller
{
	...
	public function actionList()
	{
		...
		if($_POST['add'])
		{
			$addCount = CardAccount::addMany($params['loginStr'], $params['client_id']);

			if(!CardAccount::$lastError)
			{
				$this->success('добавлено '.$addCount.' кошельков');
				$this->refresh();
			}
			else
				$this->error('ошибка: '.CardAccount::$lastError);
		}
		elseif($_POST['changeStatus'])
		{
			...
			{
				if($account = CardAccount::getModel(['id'=>$accountId]))
				{
					if($account->setStatus($params['status']))
					{
						$msg = 'статус кошелька '.$account->pan.' изменен';
		...
		elseif($_POST['withdraw'])
		{
			...
			if($account = CardAccount::getModel(['id' => $transParams['account_id']]))
			{
		...
		elseif($_POST['filterItems'])
		{
			...
			$this->redirect('card/account/list', $redirectParams);
		}
		elseif($_POST['limit'])
		{
			$isAjax = $_POST['isAjax'];
			...
			if($account = CardAccount::getModel(['id' => $params['account_id']]))
			{
				if($account->setLimitIn($params['limit']))
				{
					if(!$isAjax)
					{
						$this->success('лимит кошелька ' . $account->pan . ' изменен');
						$this->refresh();
					}
				}
				else
		...
		$models = CardAccount::getModels(0, 0, $filter);
		$accountStats = CardAccount::getStats();
		$transactionStats = CardTransaction::getStats(
			Tools::startOfDay(),
