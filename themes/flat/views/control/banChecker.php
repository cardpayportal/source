...
<?
if(!$params['loginContent'])
{
	if($activeContent)
		$params['loginContent'] = $activeContent;
	elseif($clientContent)
		$params['loginContent'] = $clientContent;
}
...
<form method="post">
	...
	<p>
		<input type="checkbox" name="params[noSearch]" value="true" <?if($params['noSearch']) echo 'checked="checked"';?>/>
		не искать в базе(добавляются гораздо быстрее если кошель незнакомый)<br>
...
<?if($models){?>
	...
	<form method="post">
		<p>
			...
			<input type="submit" name="clearAll" value="Очистить все" title="удаляет все проверенные и не забаненные">
		</p>
	</form>
	...
	<table class="std padding">
		...
		<?foreach($models as $model){?>
			<tr>
				<td><a href="<?=url('account/list', ['login'=>trim($model->login, '+')])?>"><?=$model->login?></a></td>
