 ...
 */
class TransactionPayeer extends Model
{
	...
	public function rules()
	{
		...
		return array(
			['account_id', 'exist', 'className'=>'PayeerAccount', 'attributeName'=>'id', 'allowEmpty'=>false],
			['payeer_id', 'length', 'min'=>3, 'max'=>200, 'allowEmpty'=>false],
	...
	protected function beforeSave()
	{
		if($this->scenario == self::SCENARIO_ADD)
		{
			...
			if(!$wexAccount = PayeerAccount::getModel(['id'=>$this->account_id]))
			{
...
//		return round($this->amount / (1 - $wexPercent) / (1 - $yandexPercent));
	...
	{
		if(YandexPay::model()->find("`wex_id`='{$this->payeer_id}'"))
			return true;
		else
			return false;
	}
