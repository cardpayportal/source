...
$this->title = 'Sim платежи';
?>

<h1><?=$this->title?></h1>

<?if($payUrl){?>
	<a href="<?=url('sim/transaction/list')?>">назад</a>
...
<form method="post">
	<strong>поиск по orderId:</strong> <?//?>
	<p>
...
<div>
	<form method="post" action="<?=url('manager/newYandexPay')?>">
		с <input type="text" name="params[dateStart]" value="<?=$filter['dateStart']?>" />
		до <input type="text" name="params[dateEnd]" value="<?=$filter['dateEnd']?>" />
