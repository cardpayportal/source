 ...
 * @var array $params
 * @var string $payUrl
 * @var ExchangeYadBit[] $models
 * @var array $statsYandex
 * @var array $statsWex
 */

$this->title = 'Обмен яндекс';
?>
...
<?if($payUrl){?>
	...
	<a href="<?=url('manager/exchangeYandexBit')?>">назад</a>
...
<?}else{?>
	<form method="post" action="<?=url('manager/exchangeYandexBit')?>">
...
<div>
	<form method="post" action="<?=url('manager/exchangeYandexBit')?>" style="display: inline">
	...
	<table class="std padding">
		...
		<?foreach($models as $model){?>
			<tr>
				...
				<td>
					<?if($model->status == ExchangeYadBit::STATUS_SUCCESS){?>
						<span class="success"><?=$model->statusStr?></span>
						<br><?=$model->datePayStr?>
					<?}elseif($model->status == ExchangeYadBit::STATUS_ERROR){?>
						<span class="error"><?=$model->statusStr?></span>
						<br>(<?=$model->error?>)
					<?}elseif($model->status == ExchangeYadBit::STATUS_WORKING){?>
						<span class="accountTransit"><?=$model->statusStr?></span>
					<?}elseif($model->status == ExchangeYadBit::STATUS_WAIT){?>
				...
				<td>
					<?if($model->mark == ExchangeYadBit::MARK_CHECKED){?>
